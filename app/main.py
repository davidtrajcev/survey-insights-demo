...
def build_team_comparison_chart_data(safe_report: dict, focus_category: str) -> dict:
    ...
    for unit in candidate_units:
        ...
        if score is None:
            continue
        scored_units.append(
            (unit.get("org_unit_name"), score, unit.get("manager_public_id"))
        )
    ...
    # Parallel to labels: the public id of the manager whose dashboard a bar links
    # to (or null). Public ids keep the sequential employee id out of the URL.
    manager_ids = [manager_public_id for _, _, manager_public_id in scored_units]
...
def my_area(request: Request, db: Session = Depends(get_db)):
    ...
    if current_employee.is_manager:
        return RedirectResponse(
            url=f"/manager/{current_employee.public_id}", status_code=303
        )
...
@app.get("/manager/{manager_public_id}")
def manager_dashboard(
    ...
    # The URL carries an opaque public id; resolve it to the internal employee id.
    target_manager = (
        db.query(Employee).filter(Employee.public_id == manager_public_id).first()
    )
