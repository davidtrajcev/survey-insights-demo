...
class Employee(Base):
    ...
    # Opaque public identifier used in URLs (e.g. /manager/<public_id>) so the
    # sequential employee id is never exposed.
    public_id = Column(
        String, unique=True, nullable=False, index=True, default=lambda: str(uuid.uuid4())
    )
