...
# Department managers (Engineering, Revenue, Customer Operations). Left unsubmitted
# on the live pulse so they have an open survey to take — one per leadership team,
# so each team still reaches its response target.
DEPARTMENT_MANAGER_CODES = frozenset({"E001", "E002", "E029"})
...
def create_participants_and_responses(
    ...
    score_fn=None,
    pending_codes: frozenset = frozenset(),
):
    """
    ...
    for org_key, org_memberships in memberships_by_org.items():
        ...
        for index, membership in enumerate(org_memberships):
            # Codes in pending_codes are eligible but left unsubmitted (e.g. so a
            # manager has an open survey to take live); the target is filled from
            # the remaining members so team visibility is unaffected.
            is_pending = membership.employee.employee_code in pending_codes
            has_submitted = (not is_pending) and submitted_count < target_response_count
            if has_submitted:
                submitted_count += 1
    ...
    try:
        ...
        for month_index in range(ENPS_PULSE_COUNT):
            ...
            create_participants_and_responses(
                ...
                # On the live (active) pulse, leave the department managers
                # unsubmitted so they have a survey to take; their leadership team
                # still hits its target from the other members.
                pending_codes=DEPARTMENT_MANAGER_CODES if is_latest_pulse else frozenset(),
            )
